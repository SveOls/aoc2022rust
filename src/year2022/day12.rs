...
impl Run for Aoc<2022, 12> {
    fn parta(&self) -> Result<AocResult, AocError> {
        ...
        while let Some((pos, i)) = queue.pop_front() {
            ...
            if pos[0] != 0 {
                if (data[pos[0] - 1][pos[1]] as u8).saturating_sub(data[pos[0]][pos[1]] as u8) <= 1
                {
                    queue.push_back(([pos[0] - 1, pos[1]], i + 1))
                }
            }
            if pos[1] != 0 {
                if (data[pos[0]][pos[1] - 1] as u8).saturating_sub(data[pos[0]][pos[1]] as u8) <= 1
                {
    ...
    fn partb(&self) -> Result<AocResult, AocError> {
        ...
        while let Some((pos, i)) = queue.pop_front() {
            ...
            if pos[0] != 0 {
                if (data[pos[0]][pos[1]] as u8).saturating_sub(data[pos[0] - 1][pos[1]] as u8) <= 1
                {
                    queue.push_back(([pos[0] - 1, pos[1]], i + 1))
                }
            }
            if pos[1] != 0 {
                if (data[pos[0]][pos[1]] as u8).saturating_sub(data[pos[0]][pos[1] - 1] as u8) <= 1
                {
                    queue.push_back(([pos[0], pos[1] - 1], i + 1))
                }
            }
